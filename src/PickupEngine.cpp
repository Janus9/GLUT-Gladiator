...
namespace pickups {
    ...
    glm::mat4 Engine::viewProjectionMatrix;
    Vec2f Engine::cameraPosition;

    void Engine::setViewProjectionMatrix(const glm::mat4 &_viewProjectionMatrix) {
        viewProjectionMatrix = _viewProjectionMatrix;
    }

    void Engine::setCameraPosition(const Vec2f &_cameraPosition) {
    ...
    void Engine::init(
        ...
        shader.initShader("shaders/pickup_manager/vertex.vs","shaders/pickup_manager/fragment.fs");
    ...
    bool Engine::add(const Vec2f &pos, pickups::type type, float value) {
    ...
    void Engine::logDisk() const {
        ...
        log << "Memory Stride: " << sizeof(pickups::serial_data) << "B" << "\n";
        ...
        while (pickups > 0) {
            std::vector<pickups::serial_data> buffer(std::clamp(pickups, 0, BUFFER_SIZE));
            file.read(reinterpret_cast<char*>(buffer.data()), buffer.size() * sizeof(pickups::serial_data));
            ...
            for (int i = 0; i < pickupsRead; i++) {
                ...
                std::streampos pos = startPos + static_cast<std::streamoff>(sizeof(pickups::serial_data) * p.id);
    ...
    void Engine::readFromFileWorker() {
        ...
        while (pickups > 0) {
            ...
            for (const auto &p : buffer) {
                ...
                pickup.type = static_cast<pickups::type>(p.type);
    ...
    void Engine::saveToFileWorker() {
        ...
        while (pickups > 0) {
            ...
            for (size_t i = 0; i < buffer.size(); i++) {
                pickups::serial_data &p = buffer[i];
    ...
    void Engine::cleanDeadFromFileWorker() {
        ...
        while (pickups > 0) {
            ...
            readPos += std::streamoff(buffer.size() * sizeof(pickups::serial_data));

            // Write alive pickups back in
            file.seekp(writePos);
            file.write(reinterpret_cast<const char*>(buffer.data()), aliveCount * sizeof(pickups::serial_data)); 
